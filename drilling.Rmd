...
emapper %>%
  ...
  mutate(category = case_when(
    ...
    grepl("soeB", gene) ~ "08: Sulfite dehydrogenase (soeB)", # soeB: soeAC not present
    grepl("sorB", gene) ~ "08: Sulfite dehydrogenase (sorB)",
    # N metabolism
    grepl("nif[K,D,H]", gene) ~ "09: Nitrogen fixation (nifKDH)", # nifKDH
    grepl("nrf[A,H]", gene) ~ "13: Dissimilatory nitrate red. (nrfAH)", # nrfAH
    # C fixation
    grepl("1.2.7.4", ec) ~ "16: CO dehydrogenase (cooS)", # cooS
    ec == "4.1.1.39" ~ "18: RuBisCO", # PRK
    ec == "2.7.1.19" ~ "19: Phosphoribulokinase (cfxP)",
    grepl("1.17.1.10", ec) ~ "21: Formate dehydrogenase (fdhABF)",
    # H2 metabolism
    grepl("1.12.1.2", ec) ~ "22: NAD-reducing hydrogenase (hoxfHY)",
    grepl("1.12.99.6", ec) ~ "23: NiFe hydrogenase (hydAB, hoxGK)",
    grepl("1.12.2.1", ec) ~ "23: NiFe hydrogenase (hydAB, hoxGK)",
    grepl("1.12.7.2", ec) ~ "23: NiFe hydrogenase (hydAB, hoxGK)" 
  )) %>% 
