...
i <- prokka %>%
  mutate(category = case_when(
    ...
    # H2 metabolism
    ec %in% c("1.12.99.6","1.12.2.1","1.12.7.2") ~ "21: Hydrogen ox. (hydAB, hoxGK)",
    ec == "1.12.1.2" ~ "22: NAD-red. hydrogenase (hoxHFUY)"    
  )) %>% 
...
t <- kegg %>%
  mutate(reference = case_when(
    ...
    module == "M00377" ~ "Red. acetyl-CoA pathway",    
    # C metabolism
...
i <- c("M00596: Dissimilatory sulfate red.",
       ...
       "M00531: Assimilatory nitrate red.",
       "M00530: Dissimilatory nitrate red.",
       "M00165: Calvin cycle",
       "M00166: Red. PP cycle (R5P>G3P)",
       "M00167: Red. PP cycle (G3P>R5P)", 
       "M00173: Reductive citrate cycle",
       "M00377: Red. acetyl-CoA pathway",
       "M00001: Glycolysis",
       "M00307: Pyruvate ox.",
       "M00009: TCA cycle"
       ) %>% rev()
...
p4 <- k %>%
  ...
  geom_tile(colour = "white") +
  labs(x = "", y = "", fill = "") +
  scale_fill_gradient(low = "#C5ECFD", high = "#034968", guide = "none") +
  scale_x_discrete(position = "top") + theme_tidy(ratio = NULL) + coord_equal() +
  theme(
    axis.ticks.x = element_blank(),
    axis.ticks.y = element_blank(),
...
```{r}
ggsave("../figures/heatmap.pdf", width = 16, height = 18, units = "cm")
```
