...
seqtab <- read_tsv("data/ampliseq/ASV_table.tsv.gz", col_types = 'cci') %>% 
  group_by(sample) %>% 
  mutate(relab = (count / sum(count)) * 100) %>% 
  ungroup() %>% 
  mutate(sample = factor(sample, levels = c("P28309_1061","P28309_1062","P28309_1063","P28309_1064")))

gtdb <- read_tsv("data/ampliseq/ASV_tax_species.tsv.gz", col_types = cols(.default = col_character())) %>%
...
euk <- read_tsv("data/ampliseq/ASV_table_454F.tsv.gz", col_types = 'cci') %>% 
  group_by(sample) %>%
  mutate(relab = count / sum(count)) %>% ungroup()

protist <- read_tsv("data/ampliseq/ASV_tax_454F.tsv.gz", col_types = cols(.default = col_character()))
```
...
coverm <- read_tsv("data/mag/coverm.tsv.gz", col_types = 'cdc') %>%
...
checkm <- read_tsv("data/mag/bin-stats-checkm.tsv.gz", show_col_types = F)
...
bintax <- read_tsv("data/mag/gtdbtk.summary.tsv.gz", col_types = 'ccccccccc') %>% 
  mutate(phylum = case_when(
    phylum == "Proteobacteria" ~ "Pseudomonadota",
    phylum == "Actinobacteriota" ~ "Actinomycetota",
    phylum == "Firmicutes" ~ "Bacillota",
...
emapper <- read_tsv('data/mag/emapper.tsv.gz', show_col_types = 'ccddccccc')

prokka <- read_tsv('data/mag/prokka.tsv.gz', col_types = 'ccdcccc')

kegg <- read_tsv("data/mag/keggmapper.tsv.gz", col_types = 'cccc')

ani <- read_tsv("data/mag/audaxviator_ani.tsv", col_types = 'ccd')

mw <- read_tsv("data/mag/metabolicweight.tsv", col_types = 'ccd') %>% 
...
p1 <- seqtab %>%
  ...
  theme_bw() + guides(fill = guide_legend(reverse = T, ncol = 2)) +
  theme(
...
p2 <- nmds.scores %>%
  ...
  geom_point() +
  theme_bw() +
  ggrepel::geom_label_repel(aes(label = sample), color = "black",
                  size = 7/.pt) +
...
emapper %>%
  mutate(category = case_when(
    ...
    grepl("apr[A,B]", name) ~ "04: Adenylylsulfate reductase (aprAB)", # aprAB
    grepl("asr[A-C]", name) ~  "05: Anaerobic sulfite reductase (asrABC)", # asrABC
    grepl("sox[X,Y,Z,B]", name) ~ "07: Thiosulfate ox. (soxABXYZ)", # soxA: soxBCD not present
    name == "soeB" ~ "08: Sulfite dehydrogenase (soeB)", # soeB: soeAC not present
    name == "sorB" ~ "08: Sulfite dehydrogenase (sorB)",
    # N metabolism
    ec == "1.18.6.1" ~ "09: Nitrogen fixation (nifKDH)", # nifKDH
    grepl("nrf[A,H]", name) ~ "13: Dissimilatory nitrate red. (nrfAH)", # nrfAH
    # C fixation
  ...
  group_by(genome, name) %>% summarise(n = n(), .groups = "drop") %>%
  inner_join(bintax, by = "genome") %>% #filter(genome %in% names & n > 1) %>%
  select(taxon, name, n, genome) %>%
  filter(name != "-" & n > 1)
```
...
mg <- emapper %>%
  mutate(category = case_when(
    ...
    grepl("nrf[A,H]", name) ~ "22: Dissimilatory nitrate red. (nrfAH)", # nrfAH
    # C fixation
    ec ==  "1.2.7.4" ~ "16: CO dehydrogenase (cooS)", # WL pathway 
    ec =="2.3.1.169" ~ "17: Acetyl-CoA synthase (acs)", # WL pathway 
    ec == "4.1.1.39" ~ "18: RuBisCO", # Calvin cycle
...
p2 <- mg %>%
  ...
  labs(x = "", y = "", fill = "") +
  scale_x_discrete(position = "top") + theme_bw() + coord_equal() +
  theme(
    plot.margin = margin(),
    axis.ticks.x = element_blank(),
    axis.ticks.y = element_blank(),
...
k <- emapper %>%
  mutate(reference = case_when(
    ...
    grepl("K03407", ko) ~ "Chemotaxis",
    grepl("K03408", ko) ~ "Chemotaxis",
    grepl("K03409", ko) ~ "Chemotaxis",
    grepl("K03410", ko) ~ "Chemotaxis",
    grepl("K03411", ko) ~ "Chemotaxis",
    grepl("K03412", ko) ~ "Chemotaxis",
    grepl("K03413", ko) ~ "Chemotaxis",
    grepl("K03414", ko) ~ "Chemotaxis",
    grepl("K03415", ko) ~ "Chemotaxis",
    grepl("K00575", ko) ~ "Chemotaxis",
    grepl("K13924", ko) ~ "Chemotaxis",
    grepl("K03406", ko) ~ "Chemotaxis", # MCP
    grepl("K05874", ko) ~ "Chemotaxis", # tsr
    grepl("K05875", ko) ~ "Chemotaxis", # tar
    grepl("K05876", ko) ~ "Chemotaxis", # trg
    grepl("K05877", ko) ~ "Chemotaxis", # tap
    grepl("K03776", ko) ~ "Chemotaxis", # Aer
    # Flagellar assembly
    grepl("fli", name) ~ "Flagellar assembly",
    grepl("flg", name) ~ "Flagellar assembly",
    grepl("mot", name) ~ "Flagellar assembly"
  )) %>%
  filter(!is.na(reference)) %>%
  select(genome, reference, ko) %>%
  ...
  rbind(list(genome = 
               c("MEGAHIT-MetaBAT2Refined-VK-3516-cosc-f1_S23_L002.11", 'MEGAHIT-MetaBAT2Refined-VK-3516-cosc-f3_S30_L002.24'), reference = c("Chemotaxis", 'Chemotaxis'), n = c(0,0))) %>%
  # Assign a value for every genome-module combination
